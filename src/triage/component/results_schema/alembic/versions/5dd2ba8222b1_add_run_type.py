...
def upgrade():
    ...
    op.execute(text("UPDATE triage_metadata.experiment_runs SET run_type='experiment' WHERE run_type IS NULL"))
    ...
    op.execute(text("ALTER TABLE triage_metadata.experiment_runs RENAME TO triage_runs"))
    ...
    op.execute(text("CREATE TABLE triage_metadata.deprecated_models_built_by_experiment AS SELECT model_id, model_hash, built_by_experiment FROM triage_metadata.models"))
...
def downgrade():
    op.execute(text("ALTER TABLE triage_metadata.triage_runs RENAME TO experiment_runs"))
